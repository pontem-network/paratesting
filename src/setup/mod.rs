...
pub async fn find_in_buf_reader(reader: BufReader<&mut std::fs::File>, expected: String) -> Result<bool, BoxErr> {
	let res = reader.lines()
	                .find(|s| {
		                println!("{:?}", s);
		                s.as_ref().map(|s| s.trim().contains(&expected)).unwrap_or(false)
	                })
	                .is_some();
