...
pub fn deserialize_str<T: AsRef<str>, D: DeserializeOwned>(s: T) -> Result<D, impl std::error::Error> {
	serde_yaml::from_str(s.as_ref()).and_then(|value| merge_keys_serde(value).map_err(serde::de::Error::custom))
	                                .and_then(serde_yaml::from_value)
}

pub fn deserialize<R: Read>(s: R) -> Result<TestSuitCfg, impl std::error::Error> {
	serde_yaml::from_reader(s).and_then(|value| merge_keys_serde(value).map_err(serde::de::Error::custom))
	                          .and_then(serde_yaml::from_value)
}

pub fn serialize_str(doc: &TestSuitCfg) -> Result<String, impl std::error::Error> { serde_yaml::to_string(doc) }
